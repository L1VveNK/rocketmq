...
public class KVConfigManager {
    ...
    //configTable 是 HashMap 类型的，那就存在多线程并发问题。可以看到 KVConfigManager 使用 ReentrantReadWriteLock 读写锁来保证并发安全。
    ...
    public void load() {
        ...
        if (content != null) {
            //读取 ${user.home}/namesrv/kvConfig.json 中的配置内容，然后放到本地内存表 configTable 中
    ...
    public void putKVConfig(final String namespace, final String key, final String value) {
        try {
            //更新时先加写锁，再更新配置表，写锁与读锁互斥，这期间读将被阻塞
    ...
    public void persist() {
        try {
            //持久化是加的读锁，因为写磁盘一般比写内存要耗时，如果这一步也加写锁，那么写锁阻塞的时间就会更长，阻塞读配置的时间也会更长。
            //这里通过分段加锁的方式，在写内存时加写锁，在写磁盘时加读锁，减小了锁的粒度，提升锁的性能。
